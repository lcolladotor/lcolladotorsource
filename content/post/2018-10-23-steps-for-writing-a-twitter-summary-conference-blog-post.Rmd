...
Note that my blog is powered by `r CRANpkg('blogdown')` `r citep(bib[['blogdown']])` which has the function `blogdown::shortcode()` for embedding tweets. For example, `blogdown::shortcode("tweet", user="lcolladotor", id="1054380320533950464")` generates the `hugo` syntax code which then `hugo` renders into the tweet itself.

`r blogdown::shortcode("tweet", user="lcolladotor", id="1054380320533950464")`
